...
<html>
    <head>
		...
		<style>
			...
			tr.redRow td:first-child {
				border-left: 7px solid red;
			}

			tr.redRow td:last-child {
    ...
    <body>
		...
		<div class='search_selector_div'>
			<!--select id='search_selector'>
				<?php foreach ($search_id as $id => $title) : ?>
				<option value='<?= $id ?>' <?= ($id == $sid ? ' SELECTED ' : '') ?>><?= $title ?></option>
				<?php endforeach; ?>
			</select-->
			...
			<!--button id='switch_off'>Start search</button-->
			<?php if (true || isset($_GET['d'])) { ?>
		...
		<script>
			...
			function updateSearch() {
				...
				timerObj = setInterval(updateSearch, timer.value * 1000);
				$.ajax({
					url: 'proxy.php?sid='+ <?= $sid ?>,
					type: 'post',
					data: {
						sid: <?= $sid ?>,
						paramed: false
			...
			function parseData(data) {
				...
				for (var i = 0, red_row = 0; i < data.count && i < 100; i++) {
					//if ($.inArray(data.listings[i].listingId, prevListing) >= 0) {
					if (data.listings[i].newListing) {
						...
						tableRows += '<tr class="jsListingRow 1largeTableRow newLoaded';
						hasNewLoaded = true;
					}
					if (false && data.listings[i].newListing === true) {
						tableRows += ' newRow';
						if (false && red_row < 1) {
							tableRows += ' redRow';
						}
